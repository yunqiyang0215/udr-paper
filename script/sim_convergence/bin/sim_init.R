...
# Making each simulation truth as different as possible
p <- par["p"] # data dimension
n_train <- par["n_train"]
param = list(w = rep(1/K, K), U = list(), V = diag(p))


param$U <- sim_U_true(p, s = 5, identity = TRUE, partial_sharing = FALSE, equal_effect = TRUE, 
                      num_singleton = 1, num_unconstrained = 7)

# Simulate data 
dat= simulate_mixture_ebnm(n_train, param$w, param$U, param$V)
...
f0 = ud_init(dat$X, V = param$V, U_scaled = NULL, U_unconstrained = U.init, n_rank1 = 0)
